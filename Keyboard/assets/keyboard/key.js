		...
		if (this.config.id === 'ShiftLeft' || this.config.id === 'ShiftRight') {
			...
			this.domElement.addEventListener('click', () => {
				if (
					...
					imported.textArea.erase(this.config.id);
...
document.addEventListener('keydown', (e) => {
	...
	if (key) {
		...
		if (key.config.id.substring(0, 5) === 'Shift' && !fired) {
			key.activate();
			fired = !fired;
		}
	} else {
		throw new Error('This key is not defined on the keyboard, please use the keyboard layout as a guide');
	}
});
